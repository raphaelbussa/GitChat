 ...
 */
public class LoginGitterActivity extends BaseActivity {
            ...
            if (url.startsWith(Constants.API_GITTER_REDIRECT_URI)) {
                HttpRequestClient.with(LoginGitterActivity.this).getCode(url, new FutureCallback<CodeResponse>() {
                    ...
                    public void onCompleted(Exception e, CodeResponse result) {
                        ...
                        if (result.isSuccess()) {
                            HttpRequestClient.with(LoginGitterActivity.this).getGitterToken(result.getCode(), new FutureCallback<TokenResponse>() {
                                @Override
                                public void onCompleted(Exception e, TokenResponse result) {
                                    if (result.getAccess_token() != null && result.getToken_type() != null) {
                                        ...
                                        UserFactory.with(LoginGitterActivity.this, UserFactory.TYPE.GITTER).logIn(token, new UserFactory.UserCallBack() {
                                            @Override
                                            public void onError() {
                                                showError();
                                            }
                                            ...
                                            public void onSuccess(UserBean user) {
                                                Navigator.with(LoginGitterActivity.this)
                                                        .build()
